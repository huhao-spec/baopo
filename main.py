...
class CameraWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.label_cameratime.setScaledContents(True)  # 图片自动拉伸到 QLabel 大小
        ...
        self.ui.progressBar_yr.setMaximum(130)
        ...
        self.ui.progressBar_start.setMaximum(600)
        ...
        self.ui.progressBar_cx.setMaximum(130)
        ...
        self.timer_jilu.setInterval(1000)  # 1秒钟更新一次
        self.timer_jilu.timeout.connect(self.jilu_dingshi)

    def update_frame(self):
        self.plc = Smart200('192.168.3.180')
        self.plc.ConnectPLC()
        self.main_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        url = 'rtsp://admin:a12345678@192.168.3.185'
        mp = 'D:/BaiduNetdiskDownload/new_video.mp4'

        self.camera = cv2.VideoCapture(0)
        fourcc = cv2.VideoWriter.fourcc('m', 'p', '4', 'v')
        self.mp4_path = 'D:/BaiduNetdiskDownload/2/' + str(datetime.date.today()) + '.avi'
        self.mp4_name = str(datetime.date.today()) + '.avi'
        print(self.mp4_name)

        self.out = cv2.VideoWriter(self.mp4_path, fourcc, 20, (640, 480))
    ...
    def update_img(self):
        ...
        # tem = str(round(self.tem, 2))
        # self.ui.lineEdit_tem.setText(tem)
        # text = '温度为：'+tem
        self.tem = 100
        ...
        # Density = self.read_f(48, 49, 50, 51)
        ...
        frame_text = cv2.putText(self.frame, text, (200, 100), cv2.FONT_HERSHEY_COMPLEX, 2.0, (100, 200, 200), 5)
        self.out.write(frame_text)
        ...
        if ret:
            ...
            if self.timer1.isActive():
                # 调用深度学习检测
                with torch.no_grad():
                    img = Image.fromarray(self.frame.astype('uint8')).convert('RGB')
    ...
    def jilu_caozuo(self):
        ...
        cur = conn.cursor()
        sql = "INSERT INTO tempurter (main_time, tem, pot_num,state) VALUES ('%s', %s, 1,1)" %(create_time1, self.tem)
        cur.execute(sql)
        conn.commit()
        cur.close()
        conn.close()
    ...
    def updateProgress3(self):
        ...
        if value_wash == 2:
            self.ui.progressBar_start.setValue(600)
            self.timer1.stop()
            self.ui.progressBar_yr.setValue(130)
            self.timer0.stop()
            # 结束信号
            self.plc.WriteData('VB', 20.0, 0)
        if value_wash == 130:
            self.timer2.stop()
    ...
    def updateProgress2(self):
        ...
        conn = database_connect()
        cur = conn.cursor()
        sql = "INSERT INTO tempurter (main_time, tem, pot_num,state) VALUES ('%s', %s, 1 ,0)" %(create_time, self.tem)
        cur.execute(sql)
        conn.commit()
        cur.close()
        conn.close()
    ...
    def updateProgress1(self):
        ...
        if value_yr == 1:
            self.plc.WriteData('VB', 77.0, 0)
            self.plc.WriteData('VB', 19.0, 1)
        if value_yr == 30:
            ...
            conn = database_connect()
            cur = conn.cursor()
            sql = "INSERT INTO ph_density_info (main_time, PH, Density,pot_num) VALUES ('%s', %s,%s, 1)" % (self.main_time, PH,Density)
            try:
                cur.execute(sql)
                conn.commit()
        ...
        if value_yr == 130:  # 预热操作 130
            self.timer0.stop()
            self.timer1.start()
            self.plc.WriteData('VB',20.0,1)
    ...
    def init_plot(self):
        ...
        self.curve.setData([0, 60], [40, 150])
    ...
    def goreturn(self):
        ...
        conn = database_connect()
        cur = conn.cursor()
        sql = "INSERT INTO vedio_path (mp4_name, mp4_path) VALUES ('%s', %s)" %(self.mp4_name, self.mp4_path)
        cur.execute(sql)
        conn.commit()
        cur.close()
        conn.close()
